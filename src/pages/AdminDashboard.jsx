...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const storedBikes = JSON.parse(localStorage.getItem('rrb_bikes') || '[]')
  ...
  const deleteBike = (id) => {
    ...
    localStorage.setItem('rrb_bikes', JSON.stringify(updated))
    setBikes(updated)
  }
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={logout}
            className="bg-gray-800 text-red-500 px-4 py-2 rounded font-bold hover:bg-gray-700 transition-all duration-300 border border-red-500">
            Logout
          </button>
        </div>
        ...
        <div className="flex gap-2 mb-8 flex-wrap">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`px-5 py-2 rounded font-bold transition-all duration-300 ${
                activeTab === t.id ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'
              }`}>
              {t.label}
            </button>
          ))}
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {messages.slice(0, 3).map(m => (
                <div key={m.firestoreId}
                  className={`bg-gray-900 p-4 rounded-lg border ${m.read ? 'border-gray-800' : 'border-red-500'}`}>
                  <div className="flex justify-between">
                    <p className="text-white font-bold">{m.name}
                      <span className="text-gray-400 text-sm"> — {m.phone}</span>
                    </p>
                    <div className="flex gap-1">
                      {m.type === 'exchange' && (
                        <span className="bg-green-600 text-white text-xs px-2 py-1 rounded font-bold">🔄 Exchange</span>
                      )}
                      {!m.read && (
                ...
                <tbody>
                  {bikes.map(b => (
                    <tr key={b.id} className="border-b border-gray-800">
                      ...
                      <td className="py-3 pr-4">
                        <span className={`px-2 py-1 rounded text-xs font-bold text-white ${b.type === 'sell' ? 'bg-red-600' : 'bg-green-600'}`}>
                          {b.type}
                        </span>
                      </td>
                      <td className="py-3">
                        <button onClick={() => setEditBike(b)}
                          className="bg-yellow-600 text-black px-3 py-1 rounded text-sm ml-2">Edit</button>
                        <button onClick={() => deleteBike(b.id)}
                          className="bg-red-900 text-red-400 px-3 py-1 rounded text-sm hover:bg-red-700 hover:text-white transition-all duration-300 ml-1">Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {activeTab === 'add' && (
          <div className="bg-gray-900 p-8 rounded-lg border border-gray-800 max-w-2xl">
            ...
            <button onClick={addBike} disabled={uploading}
              className={`mt-6 w-full py-3 rounded font-black text-lg transition-all duration-300 ${
                uploading ? 'bg-gray-600 text-gray-400' : 'bg-red-600 text-white hover:bg-red-700'
              }`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-xl font-black text-white mb-4">
                ...
                {exchangeCount > 0 && (
                  <span className="ml-2 bg-green-600 text-white text-xs px-2 py-1 rounded font-bold">
              ...
              <div className="flex flex-col gap-3">
                {messages.map(m => (
                  <div key={m.firestoreId} onClick={() => setSelectedMsg(m)}
                    className={`bg-gray-900 p-4 rounded-lg border cursor-pointer hover:border-red-500 transition-all duration-300 ${
                      selectedMsg?.firestoreId === m.firestoreId ? 'border-red-500' : m.read ? 'border-gray-800' : 'border-yellow-500'
                    }`}>
                    <div className="flex justify-between">
                      <p className="text-white font-bold">{m.name}</p>
                      <div className="flex gap-1">
                        {m.type === 'exchange' && (
                          <span className="bg-green-600 text-white text-xs px-2 py-1 rounded font-bold">🔄</span>
                        )}
            ...
            {selectedMsg && (
              <div className="bg-gray-900 p-6 rounded-lg border border-gray-800">
                ...
                {selectedMsg.type === 'exchange' && (
                  <span className="bg-green-600 text-white text-xs px-2 py-1 rounded font-bold mt-2 inline-block">
                    🔄 EXCHANGE REQUEST
                  </span>
                )}
                <div className="bg-gray-800 p-4 rounded mt-4">
                  <p className="text-white">{selectedMsg.message}</p>
                </div>
                ...
                <button onClick={sendReply}
                  className="bg-red-600 text-white w-full py-3 rounded font-black mt-3 hover:bg-red-700 transition-all duration-300">
                  SEND REPLY ✅
                </button>
              </div>
            )}
        ...
        {editBike && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center">
            <div className="bg-gray-900 p-6 rounded-lg w-full max-w-xl">
              ...
              <input value={editBike.name}
                onChange={(e) => setEditBike({ ...editBike, name: e.target.value })}
                className="w-full p-2 bg-gray-800 text-white mb-2" />
              <input value={editBike.price}
                onChange={(e) => setEditBike({ ...editBike, price: e.target.value })}
                className="w-full p-2 bg-gray-800 text-white mb-2" />
              <button onClick={() => {
                const updated = bikes.map(b => b.id === editBike.id ? editBike : b)
                setBikes(updated)
                localStorage.setItem('rrb_bikes', JSON.stringify(updated))
                setEditBike(null)
              }} className="bg-green-600 text-white px-4 py-2 rounded mr-2">Update</button>
              <button onClick={() => setEditBike(null)}
