  ...
  return (
    <div className="bg-gray-950 min-h-screen py-10 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-black text-white">ALL <span className="text-red-500">BIKES</span></h1>
          <div className="h-1 w-20 bg-red-500 mt-2"></div>
          <p className="text-gray-400 mt-2">{filtered.length} bikes found</p>
        </div>

        <div className="flex flex-wrap gap-4 mb-8">
          <input type="text" placeholder="Search bike..." value={search}
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 text-sm mt-2">Go to the admin dashboard to add bikes.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {filtered.map(bike => (
              <div key={bike.firestoreId}
                ...
                <div className="relative">
                  {bike.typeMedia === 'video' ? (
                    <video src={bike.img} className="w-full" style={{ aspectRatio: '16/9' }}
                      muted autoPlay loop playsInline controls />
                  ) : bike.img ? (
                    <img src={bike.img} alt={bike.name} className="w-full h-52 object-cover" />
                  ) : (
                    <div className="w-full h-52 bg-gray-800 flex items-center justify-center text-gray-500 text-4xl">🏍️</div>
                  )}
