...
function BikeDetail() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'bikes'), (snap) => {
      ...
      setAllBikes(data)
      // ✅ firestoreId बाट bike खोज्नुस्
      const found = data.find(b => b.firestoreId === id)
      setBike(found)
      setLoading(false)
    })
    return () => unsub()
    ...
    return (
      <div className="bg-gray-950 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/bikes" className="bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded font-bold">Back to Bikes</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="bg-gray-950 min-h-screen py-10 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <Link to="/bikes" className="text-red-500 font-bold hover:underline mb-6 inline-block">← Back to All Bikes</Link>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mt-4">
          <div>
            {bike.typeMedia === 'video' ? (
              <video src={bike.img} controls className="w-full rounded-lg border border-gray-800" />
            ) : bike.img ? (
              <img src={bike.img} alt={bike.name} 
              className="w-full rounded-lg border border-gray-800 
              object-cover hover:scale-105 transition duration-300" />
            ) : (
              <div className="w-full h-72 bg-gray-800 rounded-lg flex items-center justify-center text-6xl">🏍️</div>
            )}
            <span className={`inline-block mt-4 px-4 py-2 rounded font-bold text-white text-sm ${bike.type === 'sell' ? 'bg-red-600' : 'bg-green-600'}`}>
          ...
          <div>
            ...
            <a href={`https://wa.me/9779827485706?text=Hello, I am interested in ${bike.name} (Rs. ${bike.price})`}
        ...
        <div className="bg-gray-900 p-8 rounded-lg border border-gray-800 mt-10 max-w-xl">
          ...
          {sent && <div className="bg-green-600 text-white p-4 rounded mb-4 font-bold">✅ Inquiry sent! We will contact you soon.</div>}
        ...
        {/* ✅ RELATED BIKES — Firestore */}
        {allBikes.filter(b => b.firestoreId !== id).length > 0 && (
          <div className="mt-16">
            <h2 className="text-3xl font-black text-white mb-6">Related Bikes</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {allBikes.filter(b => b.firestoreId !== id).slice(0, 3).map(item => (
                <div key={item.firestoreId} className="bg-gray-900 rounded-lg overflow-hidden border border-gray-800 hover:border-red-500 transition-all">
                  ...
                  <div className="p-4">
                    ...
                    {/* ✅ firestoreId use */}
                    <Link to={`/bikes/${item.firestoreId}`}
